...
function accesValidation(globalError) {
    if (globalError.firstName === true && globalError.lastName === true && globalError.city === true) {
...
function total() {
    ...
    localStorageActualise.forEach((article) => {
        ...
        fetchData(urlProducts)
            .then(
                function recalculQuantityPrice(dataResult) {
    ...
    if (localStorageActualise.length != 0) {
        let totalQuantity = tableauLocalQuantity.reduce((a, b) => a + b);
        let emplacementTotalQuantity = document.getElementById('totalQuantity');
        emplacementTotalQuantity.innerText = totalQuantity;
        let totalPrice = tableauLocalPrice.reduce((a, b) => a + b, 0);
