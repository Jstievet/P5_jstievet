...
console.log("storageArticles", storageArticles.length);
if (storageArticles === null || storageArticles.length === 0) {
                                ...
                                if (color === verifIdColor.color) {
                                    let cuurentElementDelete =
                                        document.querySelectorAll(
                                            `[data-id="${article.id}_${article.color}"]`
                                        );

                                    cuurentElementDelete[0].setAttribute(
                                        "hidden",
                                        true
                                    );
                                    cuurentElementDelete[0].classList.add(
                                        "cart__item__hidden"
                                    );
                                    ...
                                    console.log(
                                        "after suppress cartProducts",
                                        cartProducts
                                    );
                                    if (cartProducts.length == 0) {
                                        console.log('plus de panier, remoove form price etc etc');
                                    }
                                    //supprimer la ligne de l'article sans reload
                                } else {
                                    // pas le bon id
                                }
                            }
    ...
    cityInput.addEventListener("change", function (e) {
        ...
        accesValidation(globalError);
        if (accesValidation(globalError) === true) {
            buttonSubmitValid.removeAttribute("disabled");
            buttonSubmitValid.style.opacity = "1";
        }
    });
    ...
    buttonSubmit.addEventListener("click", function (e) {
        ...
        if (globalError.firstName && globalError.lastName && globalError.city) {
